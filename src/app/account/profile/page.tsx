...
export default function ProfilePage() {
  ...
  const [email, setEmail] = useState(user?.via === "email" ? user.contact : "");
  ...
  return (
    <>
      ...
      <div className="overflow-hidden rounded-2xl border border-navy-50 bg-white shadow-soft">
        <div className="h-20 bg-linear-to-r from-navy-800 to-navy-600 sm:h-24" />
        <div className="flex flex-wrap items-end gap-x-4 gap-y-3 px-4 pb-5 sm:px-6">
          <div className="relative -mt-10">
            <Avatar
              user={user}
              className="h-16 w-16 text-lg ring-4 ring-white sm:h-20 sm:w-20 sm:text-xl"
            />
            <button
              type="button"
          ...
          <div className="mb-1 min-w-0 flex-1">
            <h2 className="truncate text-base font-bold text-navy-800 sm:text-lg">
              {user.name}
            </h2>
            <p className="truncate text-sm text-muted">{user.contact}</p>
          </div>
          <span className="mb-1 inline-flex shrink-0 items-center gap-1.5 rounded-full bg-gold-500/15 px-3 py-1 text-xs font-bold text-gold-600">
            Gold Member
          </span>
        </div>
      </div>
      ...
      <form
        onSubmit={handleSave}
        className="mt-6 rounded-2xl border border-navy-50 bg-white p-4 shadow-soft sm:p-6"
      >
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          <button
            type="submit"
            className="w-full rounded-xl bg-gold-500 px-6 py-3 text-sm font-bold text-navy-800 shadow-md transition hover:bg-gold-400 hover:shadow-lg sm:w-auto sm:py-2.5"
          >
            Save changes
          </button>
          {saved && (
